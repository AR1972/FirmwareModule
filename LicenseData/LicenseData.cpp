...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (wcscmp(argv[1], L"0") == 0) {
		...
		if(isEfi()) {
			...
			//
			int Data = 0;
			SetFirmwareEnvironmentVariable(TEXT("FailSafe"), failsafeguid, &Data, 1);
			//
			// write slp string
			//
			char slpstr[0x20] = {0};
...
Exit:
	...
	if(failsafeguid != NULL) {delete[] failsafeguid;}
